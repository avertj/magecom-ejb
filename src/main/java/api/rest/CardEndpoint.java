...
public class CardEndpoint
{
   ...
   public Response create(CardDTO dto)
   {
      ...
      em.persist(entity);
      return Response.created(UriBuilder.fromResource(CardEndpoint.class).path(String.valueOf(entity.getId())).build()).build();
   }
   ...
   {
      TypedQuery<Card> findByIdQuery = em.createQuery("SELECT DISTINCT c FROM Card c WHERE c.id = :entityId ORDER BY c.id", Card.class);
   ...
   @GET
   @Produces("application/json")
   public List<CardDTO> listAll(@QueryParam("start") Integer startPosition, @QueryParam("max") Integer maxResult)
   {
      TypedQuery<Card> findAllQuery = em.createQuery("SELECT DISTINCT c FROM Card c ORDER BY c.id", Card.class);
